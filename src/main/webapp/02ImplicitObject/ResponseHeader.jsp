...
/* 
응답헤더에 날짜를 지정하는 경우 대한민국은 세계표준시(그리니치천문대)에 +09 즉 9시간이 빠르므로 그만큼 더해줘야 정삭적인 날짜가 출력된다.
...
System.out.println(s.parse(request.getParameter("add_date")).getTime());
// 숫자형식으로 전송된 값은 정수로 변환한다.
int add_int = Integer.parseInt(request.getParameter("add_int"));
// 문자형은 그대로 사용하면 된다.
String add_str = request.getParameter("add_str");
/* <form> 태그를 통해 서버로 전송되는 값은 모두 String 타입이므로 숫자나 날짜형식의 연산이 필요한 경우에는 변환후 사용해야한다. */
...
*/
// 날짜형식 추가
response.addDateHeader("myBirthday", add_data);
