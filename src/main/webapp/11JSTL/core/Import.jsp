...
import태그 : 
	...
	include 지시어처럼 외부문서를 먼저 포함시킨후 컴파일하는 형식이 아니라, include 액션태그와 같이 컴파일이 먼저 진행된 후 결과를 포함시키게된다.
 ...
 -->
	...
	<!-- 아래와 같이 이미지를 삽입할떄는 주로 상대경로를 사용하는것이 좋다.
	만약 절대경로를 사용해야한다면 하드코딩하는것 보다 request내장객체에서 제공하는 메서드를 사용하는것이 좋다.
	웹프로그래밍은 웹서버에 배포하는것이 목적이므로 서버의 환경이 달라지면 경로까지 수정해야하는 경우 이를 최소화 할수 있도록 개발하는것이 좋다.
	...
	<!-- 컨택스트 루트명을 가져온 후 절대경로로 삽입하기(내장객체 사용) -->
