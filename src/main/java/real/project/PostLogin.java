...
public class PostLogin extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		/* Java에서 JSON데이터를 생성하기 위한 확장 라이브러리 json-simple.jar를 프로젝트에 설치한 후 아래 객체를 생성한다.
		JSON은 객체와 배열로 구분되는데 객체인 경우 JSONObject를 사용하고, 배열인 경우 JSONArray를 사용한다. */
		...
		if(dto.getName()!=null) {
			...
			json.put("result", 1);
			json.put("message", dto.getName()+"님, 로그인 성공입니다^^7");
			
			// JSON객체에 저장할 값은 아래와 같이 HTML태그도 가능한다.
			String html = "";
			html += "<table class='table table-bordered' style='width:300px;'>";
			html += "   <tr>";
			html += "      <td>"+dto.getName()+" 회원님, 반갑습니다^^7</td>";
			html += "   </tr>";
			html += "</table>";
			json.put("html", html);
		}else {
